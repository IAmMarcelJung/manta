...
class ReadyValidSourceBridge(Elaboratable):
    """
    A module for bridging an incoming 8-bit data stream to Manta's
    internal bus with proper handshaking.
    """
    ...
    def _drive_output_bus(self, m):
        ...
        with m.Else():
            m.d.comb += self.bus_o.addr.eq(0)
            m.d.comb += self.bus_o.data.eq(0)
            m.d.comb += self.bus_o.rw.eq(0)
            m.d.comb += self.bus_o.valid.eq(0)
